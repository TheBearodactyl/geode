...
namespace geode {
    ...
    public:
        ...
        static Task run(Run&& body, std::string_view name = "<Task>") {
            auto task = Task(Handle::create(name));
            std::thread([handle = std::weak_ptr(task.m_handle),
        ...
        static Task<std::vector<T*>, std::monostate> all(
            std::vector<Task<T, NP>>&& tasks, std::string_view name = "<Multiple Tasks>"
        ) {
            ...
            for (auto& taskToWait : tasks) {
                static_cast<Waiting*>(task.m_handle->m_extraData->ptr)
                    ->taskListeners.emplace_back(taskToWait.map(
        ...
        auto map(
            ResultMapper&& resultMapper, ProgressMapper&& progressMapper, OnCancelled&& onCancelled,
            std::string_view name = "<Mapping Task>"
        ) const {
            ...
            static_assert(
                ...
                "The type being mapped to must be move-constructible!"
            );
            static_assert(
                std::is_move_constructible_v<P2>,
                "The type being mapped to must be move-constructible!"
            );

            Task<T2, P2> task =
                Task<T2, P2>::Handle::create(fmt::format("{} <= {}", name, m_handle->m_name));
                ...
                task.m_handle->m_extraData = std::make_unique<typename Task<T2, P2>::Handle::ExtraData>(
                    static_cast<void*>(new EventListener<Task>(
                        [handle = std::weak_ptr(task.m_handle),
                         ...
                         onCancelled = std::move(onCancelled)](Event* event) mutable {
        ...
        auto map(
            ResultMapper&& resultMapper, ProgressMapper&& progressMapper,
            std::string_view name = "<Mapping Task>"
        ) const {
        ...
        void listen(OnResult&& onResult, OnProgress&& onProgress, OnCancelled&& onCancelled) const {
            ...
            listener->bind([onResult = std::move(onResult),
                            onProgress = std::move(onProgress),
                            onCancelled = std::move(onCancelled),
                            listener](Event* event) mutable {
                bool finished = false;
                if (auto v = event->getValue()) {
                    finished = true;
                    onResult(v);
         ...
         * The function signature should be `Task<NewType, NewProgress>(T*)`, and it will be
        ...
        auto chain(Mapper mapper, std::string_view name = "<Chained Task>") const
            ...
            task.m_handle->m_extraData = std::make_unique<typename NewTask::Handle::ExtraData>(
                ...
                static_cast<void*>(new EventListener<Task>(
                    [handle = std::weak_ptr(task.m_handle),
                     mapper = std::move(mapper)](Event* event) mutable {
                        if (auto v = event->getValue()) {
                            auto newInnerTask = mapper(v);
                            // this is scary.. but it doesn't seem to crash lol
                            handle.lock()->m_extraData =
                                std::make_unique<typename NewTask::Handle::ExtraData>(
                                    // make the second event listener that waits for the mapper's
                                    // task and just forwards everything through
        ...
        void setListener(EventListenerProtocol* listener) {
            ...
            if (m_handle->m_finalEventPosted) {
                if (m_handle->m_status == Status::Finished) {
                    queueInMainThread([handle = m_handle,
                                       listener = m_listener,
                                       value = &*m_handle->m_resultValue]() {
...
//
// The body of the coroutine is always executed on the main thread.
//
    ...
    namespace geode_internal {
        ...
        struct TaskPromise {
            ...
            std::suspend_never initial_suspend() noexcept {
                // Queue the coroutine to start on main thread
            ...
            void unhandled_exception() {
                // cancel the task and rethrow the exception
                MyTask::cancel(m_handle.lock());
